...
def resolve_attack(attacker, defender, attack_style, base_damage):
    ...
    if rng.random() > hit_chance:
        return {"type": "miss", "damage": 0.0, "base_damage": base_damage, "mitigated": 0.0, "attack_style": attack_style, "hit_chance": hit_chance}
    ...
    if attack_style == "melee":
        ...
        if def_parry > 0 and rng.random() < def_parry:
            return {"type": "parry", "damage": 0.0, "base_damage": base_damage, "mitigated": base_damage, "attack_style": attack_style, "hit_chance": hit_chance}
    ...
    if def_block > 0 and rng.random() < def_block:
        ...
        return {"type": "block", "damage": final_damage, "base_damage": base_damage, "mitigated": max(0.0, base_damage - final_damage), "attack_style": attack_style, "hit_chance": hit_chance}
    ...
    return {"type": "crit" if is_crit else "hit", "damage": final_damage, "base_damage": base_damage, "mitigated": max(0.0, base_damage - final_damage), "armor_reduction": reduction, "attack_style": attack_style, "hit_chance": hit_chance}
...
class TargetedProjectile:
    def __init__(self, render, origin, target, damage, profile, on_hit):
        ...
        orb = self.root.attachNewNode(make_sphere_approx(profile["projectile_radius"], profile["projectile_color"]))
        orb.setPos(0, 0, 0)

    def update(self, dt, hit_context=None):
        if self.target is None or not self.target.is_targetable():
            self.remove(); self.expired = True; return True
        ...
        if dist > 0: delta.normalize()
        self.pos += delta * min(dist, self.profile["projectile_speed"] * dt)
        self.root.setPos(self.pos)
        return False

    def remove(self):
        if not self.root.isEmpty(): self.root.removeNode()
